...
4. Locate the exact files relevant to the issue — don't try to explain the whole codebase, scope down to what's needed.
5. Draft the fix, then **explain the diff back to the user line by line** rather than letting them submit something neither of you fully understands.
6. Run the project's actual test suite locally (per the contribution guide — pytest, jest, cargo test, etc.) before suggesting a PR.
7. Commit the changes and push to the fork's branch.
...
1. **Automated PR Creation:** Instruct the user to run `gh pr create --title "..." --body "..."` using the drafted title and description matching the project's style.
   ...
   Some high-profile repos silently block PRs from brand-new accounts (anti-spam): `gh pr create` fails with a permissions error and the REST endpoint returns 404 even though the fork and branch exist. If the issue reporter also mentions being "blocked", don't try to work around it — pick a different repo.
