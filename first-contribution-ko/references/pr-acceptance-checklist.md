...
**일부 레포는 봇 라벨이 아니라 유지보수자 사전 승인으로 게이트를 겁니다.** dlt의 CONTRIBUTING은 첫 기여자를 `help wanted`/`good first issue` 이슈로 제한하고 나머지 전부에 명시적 유지보수자 사전 승인을 요구합니다 — 수정 제안까지 정밀하게 문서화된 `bug`(dlt #4315)도 구현 *전에* 계획 코멘트를 승인받아야 하며, 눈감고 PR을 열면 polars 스크린샷 게이트와 마찬가지로 리뷰 없이 닫힙니다. CONTRIBUTING이 승인을 요구하면 추측으로 구현하지 말고 이슈에 의도한 접근을 코멘트로 남기고 기다리세요.

**PR을 열기 전에 같은 이슈에 대한 경쟁 PR이 있는지 확인하세요.** 이슈의 Development 섹션에 연결 없이 독립적으로 머지 안 된 PR이 여러 개 경주하고 있을 수 있습니다(outlines #1998/#1996/#1997/#1980 각각에 우리 것 포함 2~3개의 겹치는 PR 발생). 이슈 타임라인 / `gh search prs` / 같은 버그를 참조하는 다른 제목을 검색하세요. 겹침이 이미 있으면 *가장 먼저* 열리고 *가장 작은* PR을 남기고, 중복을 직접 "어느 걸 살릴지" 짧게 밝히며 닫고, 살린 PR에 (먼저 열림·최소 diff·단일 이슈 링크) 포지셔닝 코멘트를 달아주세요.
...
## 0c. CONTRIBUTING을 앞서 읽어 첫 기여자 요구사항을 확인하세요 — AI 공개만으론 부족할 수 있습니다
AI PR로 넘쳐나는 일부 레포는 공개만 요구하지 않고 **로컬 검증의 증거**를 원하며, 코드가 아무리 정확해도 없으면 PR을 닫습니다. polars #28911(올바른 `DataFrame.n_unique` 수정)은 메인테이너가 CONTRIBUTING의 첫 기여자 섹션 링크 하나만 남기고 닫았습니다. 그 섹션은 이렇게 요구합니다: *"로컬에서 테스트 스위트(`make test`)를 성공시킨 스크린샷을 올려야 합니다(CI가 아니라). 스크린샷은 터미널 창 테두리가 보여야 합니다..."* — 그리고 "열려 있는 PR은 한 번에 하나만"도 요구합니다. 우리는 AI 도움을 공개하고 venv에서 로직을 검증했지만, 그 스크린샷을 만들 만큼 전체 로컬 스위트를 돌리지 않아 리뷰 전에 정책상 닫혔습니다.

PR에 노력을 들이기 전에 **레포의 CONTRIBUTING 첫 기여자 섹션을 읽고, 매 조건을 실제로 충족할 수 있는지** 확인하세요 — 공개만으론 부족합니다:

- **CI 너머로 로컬 테스트 스크린샷이나 다른 실행 증거를 요구하나요?** 그렇다면 PR을 열기 전에 정확한 명령(`make test`, `pytest` 등)을 내 기기에서 돌리고, 터미널 테두리가 보이는 제대로 된 스크린샷을 찍을 수 있는지 확인하세요. 못 돌리면(거대한 설치, 하드웨어 부족) 품질과 무관하게 정책상 닫힐 가능성이 높으니, 그러한 요구가 없는 레포를 찾거나 메인테이너에게 먼저 요건을 처리해달라고 하세요.
- **동시 오픈 PR을 제한하나요, 소유한 이슈를 요구하나요, 머지 횟수/라벨/유지보수자 사전 승인으로 게이트를 거나요?** 각 전제 조건이 원칙적으로뿐 아니라 지금 충족 가능한지 확인하세요. 특히 일부 레포(dlt 등)는 첫 기여자가 `help wanted`/`good first issue` 이슈만 다루게 하고 나머지 전부는 명시적 유지보수자 사전 승인을 요구합니다 — 정밀하게 문서화된 `bug`(dlt #4315)도 구현 전에 계획 코멘트를 승인받아야 하고, 눈감고 PR을 열면 리뷰 없이 닫힙니다. CONTRIBUTING이 승인을 요구하면 추측으로 구현하지 말고 이슈에 의도한 접근을 코멘트로 남기고 기다리세요.
- **§0b의 요약은 "못 돌렸다"로 끝나면 안 됩니다.** 증거를 요구하는 레포라면, 정말 로컬 스위트를 못 돌린다면 그렇게 말하고 제출 전에 메인테이너에게 PR이 환영받는지 먼저 물어보세요 — 작업한 뒤에 정책상 닫히게 두지 마세요.
...
- **포맷팅 린트 게이트는 테스트와 별개로 실패할 수 있습니다.** litellm #38477은 기능 검사를 전부 통과했는데 `ruff format --check` 때문에 `lint` job이 실패했습니다(내가 쓴 `extra=(...)` 리터럴이 ruff 모양이 아니었음). push 전에 변경 파일들에 그 레포가 고정한 포맷터를 *로컬에서* 돌리세요(`uvx --from ruff==0.15.3 ruff format path/to/file.py`, 또는 그 프로젝트의 자체 `ruff format` 명령) — CI와 포맷팅 수정을 왕복시키지 말고요. 내 전역 버전이 아니라 고정된 정확한 버전을 쓰세요.
