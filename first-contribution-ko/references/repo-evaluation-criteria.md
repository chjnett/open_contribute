...
## 3. PR 머지 현실 (이슈 triage 전에 — 단, 가용성 필터 다음에)

싼 것부터: 지금 열려있는 미할당 기여 라벨 이슈가 0건인 레포는 게이트 수치가 어떻든 오늘 기여를 만들 수 없고, 그 확인은 라벨당 REST 호출 한 번이면 끝납니다. 후보 묶음을 가용성으로 먼저 거른 뒤 살아남은 곳만 게이트에 넣으세요(쿼리는 SKILL.md Phase 2에 있습니다). 게이트를 먼저 돌리고 나서 이슈 풀이 비어있는 걸 발견하면 그 바퀴를 통째로 낭비합니다.
