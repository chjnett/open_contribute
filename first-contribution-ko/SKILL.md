...
2. **맞는 사람에게 갔는지 확인하세요.** `.github/CODEOWNERS`에서 파일 경로만이 아니라 그 *기능*을 소유한 팀을 찾으세요 — 커버리지가 불완전한 경우가 많고, GitHub는 매칭되는 소유자에게만 자동 요청합니다. `grafana/grafana#130614`에서 short URL 기능은 네 경로에 걸쳐 `@grafana/sharing-squad` 소유인데, 수정한 utils 파일이 어디에도 매칭되지 않아 무관한 프론트엔드 리뷰어에게 갔습니다. 이런 공백을 발견하면 팀과 이유를 적은 코멘트 하나가 압박이 아니라 메인테이너에게 유용한 정보가 됩니다.
