...
### Added
- **Search for competing PRs on the same issue before opening, and honestly consolidate when several overlap** (PR checklist §0/pr-acceptance §0b). On outlines, four of our freshly-opened PRs (#2011-#2014, addressing #1998/#1996/#1997/#1980) turned out to race earlier or later competitor PRs on the same bugs (#2007, #2008, #2020, #2021, #2022). Before submitting, search open/closed PRs whose titles or linked issues touch the same code path (e.g. `gh search prs`/issue timeline), not just the issue you're fixing. When overlap already exists: keep the PR that is *earliest* and *smallest* (or already-review-in-progress); for a genuinely duplicated later PR, close it yourself with a short comment naming the surviving PR and why, rather than leaving several unmerged duplicates to confuse the reviewer. Then post a short positioning note on the survivors (first-opened, minimal diff, single linked issue) so a maintainer can pick confidently. Identify the de-facto maintainer via the committer of recent merges (e.g. outlines merges are all committed by `Robin Picard`) and target review pings at them.
- **An open/bug-labelled issue is not necessarily eligible — some repos gate implementation on an explicit maintainer pre-approval** (PR checklist §0/§0c). dlt's CONTRIBUTING requires first-time contributors to only pick up issues labelled `help wanted` or `good first issue`; anything else (including a plainly-described `bug` with a suggested fix, dlt #4315) needs a maintainer to approve the approach *before* implementation, and PRs opened without that approval "may be closed without a detailed review" (same fate as polars §0c). Read the repo's CONTRIBUTING submission gate (labels, pre-approval, one-PR-at-a-time, screenshot/AI-proof) before writing code; when prompted, comment the intended approach on the issue and wait for approval instead of implementing speculatively or opening the PR blind.
- **A repo's lint CI can fail on formatting, not just code — run the pinned formatter locally before pushing** (PR checklist §1). litellm #38477 passed every functional test but its `lint` gate failed on `ruff format --check` because an `extra=(...)` dict literal I wrote wasn't ruff-shaped. Fix: run the exact pinned tool/version (litellm uses ruff 0.15.3) *locally* on the changed file (`uvx --from ruff==0.15.3 ruff format ...`), which collapses the block onto one line, then amend/push a new commit. Don't round-trip formatting fixes back and forth with CI. Also prefer reusing the project's lockfile command (`uv run --no-sync ruff ...`) and only pass `--no-sync` deliberately, so the checked-in formatter is the one CI uses.
...
- **A green `signed-commits` CI check does not mean the repo doesn't enforce signatures — the enforcement is often a separate branch ruleset** (PR checklist §4). On grafana, the `signed-commits` workflow passed an unsigned commit with no `required_signatures` in classic protection, but an active `Signed Commits` ruleset still blocked merge. Query rulesets (`/rulesets?target=branch`) to find the real gate. If the user already has an SSH key on GitHub, signing is a two-line local setup (`git config gpg.format ssh` + `user.signingkey` + `git commit --amend -S`), scoped to the local repo when `~/.gitconfig` is read-only, and GitHub verifies it as `verified: true` after push — no GPG needed.
- **When a reviewer points at the "second half" of the same bug, fold it into this PR instead of deferring to a follow-up** (PR checklist §6). On LLaMA-Factory #10759, the reviewer confirmed the `top_p` fix then flagged the same `or 1.0` silent-rewrite still in the SGLang engine and in `repetition_penalty` at three call sites; folding them in made four backends agree and the reviewer closed with explicit praise. Practical rules: grep the whole repo for the anti-pattern class before submitting; verify the *actual commit* each round not the summary; read the full thread before treating a review as approval; and leave what the reviewer deliberately marked to keep (e.g. `top_k ... or -1` — a faithful `0`→`-1` translation, not a rewrite), saying why.
- **A cla-assistant-style CLA must be signed by the user in a browser — there is no API/CLI route** (PR checklist §4). The bot leaves a `not_signed` badge and a `?pullRequest={n}` link; hand the user the exact URL, not the repo root. One signature covers the whole org (litellm's two PRs satisfied by one sign). After signing, the check may need a re-trigger ("Let us recheck") or an empty-amend push. CLA state is per-PR, so a later PR on the same repo may show `not_signed` until it self-detects. Spot CLA-vs-label gates by reading `.github/workflows` (mem0's `pr-gate.yml` closes PRs whose issue lacks `accepted`, separate from CLA).
- **Verify a commit's author-email attribution, not just the local git config** (PR checklist §3b). A merged PR counts toward profile achievements (Pull Shark Bronze = 2 merged PRs) only if GitHub attributes the commits — keyed to the commit's author email matching an account email. Check via the Commits API (`author_login` non-null and equal to the account confirms the email is registered; null means the contribution is silently dropped). Badges can lag merge by up to ~24h (async batch), so verify `author_login` on the merged commits right away while telling the user the badge may take a day.
- **A reviewer can legitimately widen the fix to a deployment mode your issue didn't mention — smallest change is bounded by the environments the code runs in** (PR checklist §1h). grafana #130906 unconditionally baked the user's orgId into a short-URL param; reviewer evictorero pointed out Cloud doesn't support multi-org and named the existing `isOnPrem()` helper. The fix became: on-prem carries the org, Cloud omits it, with a Cloud test — approved and merged the same day. When a reviewer names an existing helper/flag to condition on, that distinction is the spec; cover the not-default mode in a test; and note a CODEOWNERS-path approval is often the one that unlocks merge (nmarrs approved but flagged a different approver was needed after he pushed an update).
- **Disclose AI assistance in the PR body up front; undisclosed AI origin can get a PR rejected as spam even when technically fine** (PR checklist §0b). sqlfluff #8350's maintainer rejected a version-pin PR with "This is very clearly fully AI, and was not disclosed. Spam, please close." Before opening a PR against an issue a maintainer already commented on, read whether they've described the resolution — the same maintainer had noted pip-tools 7.6.1 fixed the root cause, so the pin was "unnecessary and noisy" (issue-triage Step 5). On rejection, acknowledge and correct course rather than defending the code.
- **Read the CONTRIBUTING up front for first-time-contributor requirements — AI disclosure alone may not be enough** (PR checklist §0c). polars #28911, a correct and reviewer-confirmed fix, was policy-closed by a maintainer with only a link to the CONTRIBUTING "first-time contribution" section: AI-authored PRs must attach a screenshot of a successful local `make test` (visible terminal borders), and only one open PR is allowed per author at a time. Before opening in a repo you haven't contributed to, read CONTRIBUTING/README for proof-of-execution and PR-flow rules; if the required local suite can't be run (e.g. disk-constrained environment can't do a full compile/test of the project), ask a maintainer up front in a comment how to proceed rather than submitting blind.
- **A reviewer's suggested alternative is a hypothesis too — verify it locally before applying it** (PR checklist §1h). On polars #28911, the reviewer flagged that a `struct` wrapper could cost a row-encoding pass; switching to the suggested `unique(subset=...)` delegation `panicked` on the documented expression-subset case, so the `struct` wrapper was the only correct path. A pure-Python change that passed `test-python` on ubuntu but failed only `windows-latest, 3.14` should be checked for pre-existing infra issues rather than churned.
